  ...
  context "The post vote controller" do
    ...
    context "create action" do
      ...
      should "not allow banned users to vote" do
        ...
        post_authenticated post_votes_path(post_id: @post.id), @banned, params: {:score => "up", :format => "js"}
        assert_response 403
        assert_equal(0, @post.reload.score)
      end
      ...
      should "not allow members to vote" do
        ...
        post_authenticated post_votes_path(post_id: @post.id), @member, params: {:score => "up", :format => "js"}
        assert_response 403
        assert_equal(0, @post.reload.score)
      end

      should "increment a post's score if the score is positive" do
        post_authenticated post_votes_path(post_id: @post.id), @user, params: {:score => "up", :format => "js"}
