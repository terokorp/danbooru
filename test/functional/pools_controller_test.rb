...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  context "The pools controller" do
    ...
    context "new action" do
      should "render" do
        get_authenticated new_pool_path, @user
        assert_response :success
      end
    end
    ...
    context "create action" do
      should "create a pool" do
        assert_difference("Pool.count", 1) do
          post_authenticated pools_path, @user, params: {:pool => {:name => "xxx", :description => "abc"}}
        end
      end
    end
    ...
    context "edit action" do
      should "render" do
        get_authenticated edit_pool_path(@pool), @user
        assert_response :success
      end
    end
    ...
    context "update action" do
      should "update a pool" do
        put_authenticated pool_path(@pool), @user, params: { pool: { name: "xyz", post_ids: @post.id.to_s }}
      ...
      should "not allow updating unpermitted attributes" do
        put_authenticated pool_path(@pool), @user, params: { pool: { is_deleted: true, post_count: -42 }}
    ...
    context "destroy action" do
      should "destroy a pool" do
        delete_authenticated pool_path(@pool), @mod
        @pool.reload
    ...
    context "undelete action" do
      ...
      should "restore a pool" do
        post_authenticated undelete_pool_path(@pool), @mod
        @pool.reload
    ...
    context "revert action" do
      ...
      should "revert to a previous version" do
        ...
        put_authenticated revert_pool_path(@pool), @mod, params: {:version_id => version.id}
        @pool.reload
      ...
      should "not allow reverting to a previous version of another pool" do
        ...
        put_authenticated revert_pool_path(@pool), @user, params: {:version_id => @pool2.versions.first.id }
        @pool.reload
